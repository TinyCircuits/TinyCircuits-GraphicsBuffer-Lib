...
void GraphicsBuffer::clearWindow(uint8_t x, uint8_t y, uint8_t w, uint8_t h) {  
  if(x>_xMax||y>_yMax)return;
  uint8_t x2=x+w-1;
  uint8_t y2=y+h-1;
...
#endif
void GraphicsBuffer::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) {
...
void GraphicsBuffer::drawCircle(int16_t x0, int16_t y0, int16_t radius, uint16_t color){
  int x = radius;
  int y = 0;
  int radiusError = 1 - x;
